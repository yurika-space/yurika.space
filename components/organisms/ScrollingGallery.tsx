...
interface GalleryProps {
	...
	zSpacing?: number;
	/** Maximum number of images to display (default: all images) */
	maxLength?: number;
...
function GalleryScene({
	...
	// Normalize images to objects and apply maxLength
	const normalizedImages = useMemo(() => {
		const imgs = images.map((img) =>
			typeof img === 'string' ? { src: img, alt: '' } : img
		);
		return maxLength ? imgs.slice(0, maxLength) : imgs;
	}, [images, maxLength]);
	...
	const handleWheel = useCallback(
		(event: WheelEvent) => {
			...
			// If canvas is not in viewport, allow normal page scroll
			if (!isInViewport) {
				return;
			}

			const totalRange = depthRange;
			const maxScroll = totalRange * 0.9;
			const minScroll = 0;
			...
			const atTopBoundary = scrollPosition <= minScroll;
			const atBottomBoundary = scrollPosition >= maxScroll;

			// Allow page scroll if:
			// - Scrolling up and already at the top of gallery
			// - Scrolling down and already at the bottom of gallery
			if ((scrollingUp && atTopBoundary) || (scrollingDown && atBottomBoundary)) {
				// Don't prevent default - allow page to scroll
				return;
			}

			// We're within gallery bounds, handle the scroll
			event.preventDefault();
			setScrollVelocity((prev) => prev + event.deltaY * 0.01 * speed);
		},
		[speed, scrollPosition, depthRange]
	);
	...
	useFrame((state, delta) => {
		...
		const maxScroll = totalRange * 0.9; // Max forward scroll
		const minScroll = 0; // Min backward scroll
		...
		// Clamp scroll position to boundaries (no wrapping)
		// But don't reset velocity - allow scrolling back
		if (newScrollPosition > maxScroll) {
			newScrollPosition = maxScroll;
		} else if (newScrollPosition < minScroll) {
			newScrollPosition = minScroll;
		}
		...
		// Update plane positions (no wrapping)
		planesData.current.forEach((plane, i) => {
			// Each plane maintains its initial position plus the scroll offset
			const baseZ = visibleCount > 0 ? ((depthRange / visibleCount) * i) : 0;
			plane.z = baseZ + newScrollPosition;

			// Clamp to visible range
			if (plane.z < 0 || plane.z > totalRange) {
				// Plane is outside visible range, keep it at boundaries
				plane.z = Math.max(0, Math.min(totalRange, plane.z));
			}
